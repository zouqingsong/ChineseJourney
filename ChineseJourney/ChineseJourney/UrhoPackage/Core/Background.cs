...
namespace ChineseJourney.Common.UrhoPackage.Core
{
	public class Background : Component
	{
		...
		public void Start()
		{
			...
			_frontTile = CreateTile(0);
			_rearTile = CreateTile(1);
		...
		async void RotateBackground() 
		{
			while (true)
			{
				...
				await Task.WhenAll(_frontTile.RunActionsAsync(new MoveBy(1 / BackgroundSpeed, new Vector3(0, -moveTo, -h))),
					_rearTile.RunActionsAsync(new MoveBy(1 / BackgroundSpeed, new Vector3(0, -moveTo, -h))));

				//switch tiles
				var tmp = _frontTile;
				_frontTile = _rearTile;
				_rearTile = tmp;

				_rearTile.Position = new Vector3(0, x, y);
			}
		}
		...
		Node CreateTile(int index)
		{
			...
			treeNode.Rotate(new Quaternion(0, RandomHelper.NextRandom(0, 5) * 90, 0));
			...
			tile.Rotate(new Quaternion(270 + BackgroundRotationX, 0, 0));
			tile.RotateAround(new Vector3(0, 0, 0), new Quaternion(0, BackgroundRotationY, 0));
